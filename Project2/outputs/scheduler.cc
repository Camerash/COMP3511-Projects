...
Scheduler::FindNextToRun ()
{
   ...
   switch(policy)
   {
        ...
        {
            //YOUR PROJECT2 CODE HERE
            // We put threads in order so the first one is always have the shortest burst time
            next_to_run = readyList->Remove();
            break;
        }
        case SCHED_PRIO_NP:
        {
            //YOUR PROJECT2 CODE HERE
            // We put threads in order so the last one is always have the highest priority
            next_to_run = readyList->Remove();
            break;
        }
        case SCHED_PRIO_P:
...
Scheduler::ShouldISwitch ( Thread  * oldThread, Thread * newThread )
{
   ...
   switch(policy)
   {
        ...
        {
            //YOUR PROJECT2 CODE HERE
            doSwitch = newThread->getPriority() > oldThread->getPriority();
            break;
        }
